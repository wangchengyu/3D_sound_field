...
function Field(canvas) {
  ...
  var ctx = this;
  this.speakerIcon.onload = function() {
    ctx.render();
  }
}
...
Field.prototype.render = function() {
  ...
  ctx.drawImage(this.speakerIcon, this.center.x - this.speakerIcon.width/2,
                              this.center.y - this.speakerIcon.height/2);
  ctx.fill();
  ...
  if (this.point) {
    ...
    ctx.translate(-this.manIcon.width/2, -this.manIcon.height/2);
    ctx.drawImage(this.manIcon, 0, 0);
    //ctx.drawImage(this.manIcon, this.point.x - this.manIcon.width/2,
    //                                this.point.y - this.manIcon.height/2);
    ctx.restore();
  }
  ctx.fill();
};
...
Field.prototype.handleMouseMove = function(e) {
  if (this.isMouseInside) {
    ...
    if (this.callback) {
      ...
      this.callback({x: this.center.x - this.point.x,
